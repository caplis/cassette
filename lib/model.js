...
class Model {
    ...
    static _convert(value, col) {
        let meta = {};
        if (col && typeof col.describe === 'function') {
            meta = hoek.reach(col.describe(), 'meta.0', {default: {}});
        }
        if (meta.json && typeof value === 'string') {
            try {
                value = JSON.parse(value);
            } catch (e) {
                console.log('JSON.parse failed on Model._convert:', e.message);
            }
        } else {
