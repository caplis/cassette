...
function validate_model(model, schema, cb) {
    var result = joi.validate(model, schema);
    cb(result.error, result.value);
}
...
module.exports = function Model(definition, client, data) {
    ...
    methods.save = function (args, cb) {
        ...
        validate_model(model, schema, function (err, valid_model) {
            var qargs = hoek.merge(args, {params: valid_model});
