...
let update_counter = (type, args, definition) => {
    ...
    Object.keys(args).forEach((field) => {
        if (schema[field]) {
            if (definition.primary_key.indexOf(field) === -1) {
                counter_update = [field, "=", field, UPDATE_TYPE, args[field]].join(' ');
            } else {
                primary_key[field] = args[field];
            }
        }
    });
...
// PR-REVIEW:
// - datastax driver rejects params "total + X", otherwise we could
//   create a generic update interface
/**
