...
class ModelDefinition {
    ...
    many(args, cb) {
        ...
        if (args.params.length > MAX_MANY) {
            return process.nextTick(cb, new Error('Number of params exceeds max size'));
        }
        if (typeof cb !== 'function') {
        ...
        let models_map = {};
        async.eachLimit(args.params, 20, function (key, async_cb) {
            ...
            let out = original_order.map((id) => {
                return models_map[id];
            });
            out = out.filter((item) => { return !!item; });
            cb(null, out);
        });
    }
