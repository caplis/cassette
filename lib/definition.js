...
class ModelDefinition {
    ...
    all(model_iter, options, cb) {
        if (typeof options === 'function') {
            cb = options;
            options = {};
        }
        let that = this;
        let q = cql.select({}, this);
        let model_buffer = [];
        this.client.eachRow(q.query, q.params, {prepare: true, fetchSize: options.buffer_size || 200}, (n, row) => {
            model_buffer.push(create_model(that, that.client, row));
        }, (err, res) => {
            async.each(model_buffer, model_iter, (err) => {
                if (err) {
                    return cb(err);
                }
