...
class ModelDefinition {
    constructor(table, definition, client) {
        ...
        if (secondary_keys.length && hoek.intersect(hoek.flatten(primary_key), secondary_keys)) {
    ...
    one(params, cb) {
        ...
        for (let i = 0; i < secondary_keys; i++) {
            let secondary_key = secondary_keys[i];
            let secondary_param = params[secondary_key];
            if (secondary_param) {
                key_params[secondary_key] = secondary_param;
            }
        }
