...
class TableModel extends Model {
    constructor(definition, client, data, is_new) {
        ...
        Object.defineProperty(this, '_object', {
            ...
            set: function (value) {
                Object.keys(value).forEach((k) => {
                    if (definition.schema.hasOwnProperty(k)) {
                        _object[k] = Model._convert(value[k], definition.schema[k]);
                        if (that._is_new) {
        ...
        Object.keys(definition.schema).forEach((key) => {
            ...
            if (data.hasOwnProperty(key)) {
                that._object[key] = Model._convert(data[key], definition.schema[key]);
                if (that._is_new) {
            ...
            Object.defineProperty(that, key, {
                ...
                set: function(value) {
                    ...
                    value = Model._convert(value, definition.schema[key]);
    ...
    validate(cb) {
        ...
        if (typeof cb !== 'function') {
            ...
            Object.keys(validated_model).forEach((key) => {
                ...
                that._object[key] = Model._convert(validated_model[key], that.definition.schema[key]);
            });
    ...
    save(args, cb) {
        ...
        this.validate(function (err) {
            ...
            let modified = that._get_modified_params();
            Object.keys(modified).forEach((k) => {
                // stringify values for JSON columns
                let meta = hoek.reach(that.definition.schema[k].describe(), 'meta.0', {default: {}});
                if (meta.json && typeof modified[k] !== 'string') {
                    try {
                        modified[k] = JSON.stringify(modified[k]);
                    } catch (e) {
                        // log the error and let insert fail
                        console.log('JSON.stringify failed on TableModel.save():', e.message);
                    }
                }
            });
            let qargs = hoek.merge(args, {params: modified});
    ...
    sync(cb) {
        ...
        this.client.execute(q.query, q.params, {prepare: true}, function (err, res) {
            ...
            Object.keys(res.rows[0]).forEach((k) => {
                that._object[k] = Model._convert(res.rows[0][k], that.definition.schema[k]);
            });
            that._modified = [];
            cb();
        });
