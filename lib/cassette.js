...
class Cassette {
    ...
    batch(params, cb) {
        ...
        for (let i = 0; i < params.length; i++) {
            // check model
            if (!params[i].model || !(this.is_model(params[i].model))) {
                return process.nextTick(cb, new Error('Invalid model: batch index ' + i));
            }
            // check op
            if (['save','delete','increment','decrement'].indexOf(params[i].op) === -1) {
                return process.nextTick(cb, new Error('Invalid op: batch index ' + i));
            }
            // derive query and add to batch
            let data = params[i].model._is_new ? params[i].model.validate() : params[i].model._get_modified_params();
            if (data instanceof Error) {
                return process.nextTick(cb, data);
            }
            let qargs = hoek.merge((params[i].options || {}), {params: data});
            let fn;
    ...
    // PR-REVIEW:
    // - only one place we check for this (auth-plugin) and we
    //   probably only care that object is a proper cassette-model (ie
    //   base model). If we do need TableModel, CounterModel, we can add those helpers
    //
    /**
