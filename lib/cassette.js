...
class Cassette {
    ...
    query(args, cb) {
        ...
        this.client.execute(args.query, args.params, args.options, function (err, res) {
            ...
            if (that.is_definition(args.definition)) {
                // return models if definition passed
                let M = args.definition.counter_key ? CounterModel : TableModel;
                let models = res.rows.map((row) => {
                    return new M(args.definition, that.client, row);
                });
                cb(null, models);
            } else {
                // raw result
    ...
    batch(params, cb) {
        ...
        for (let i = 0; i < params.length; i++) {
            ...
            if (typeof params[i].model[params[i].op] !== 'function') {
                return process.nextTick(cb, new Error('Invalid op for ' + hoek.reach(params[i], 'model.definition.table')  + ' at batch index ' + i));
            }
            ...
            if (this.is_counter_model(params[i].model)) {
                ...
                if (params[i].op === 'update') {
                    qargs = hoek.merge({params: data}, (params[i].options || {}));
                    cql_fn = 'update';
                } else {
                    qargs = hoek.merge((params[i].options || {}), {params: data});
                    cql_fn = params[i].op === 'save' ? 'insert' : 'delete';
                }
            }
            queries.push(cql[cql_fn](qargs, params[i].model.definition));
        }
